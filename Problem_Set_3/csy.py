...
def visualize_matches(I1, I2, matches):
    ...
    for i in range(matches.shape[0]):
        if i%5 == 0:
            ax.plot(matches[i,0],matches[i,1],  '+r')
            ax.plot( matches[i,2] + I1.size[0], matches[i,3], '+r')
            ax.plot([matches[i,0], matches[i,2]+I1.size[0]],[matches[i,1], matches[i,3]], 'b')
    # save the image
    ax.axis('off')
    plt.show()
    # plt.savefig('house_matches.png', bbox_inches='tight', pad_inches=0)
...
def normalize_points(pts):
    ...
    normalized_pts = np.matmul(T, np.concatenate([pts, np.ones((pts.shape[0], 1))], axis=1).T).T
...
def visualize_fundamental(matches, F, I1, I2):
    ...
    # remove the axis
    ax.axis('off')
    plt.savefig('house.png', bbox_inches='tight', pad_inches=0)
...
print("the residual of library_F is: ", evaluate_fundamental(library_matches, library_F))
...
print("the residual of lab_F is: ", evaluate_fundamental(lab_matches, lab_F))
...
def calc_projection(points_2d, points_3d):
    ...
    # pad points_3d with 1
    points_3d = np.concatenate([points_3d, np.ones((points_3d.shape[0], 1))], axis=1)
    # pad points_2d with 1
    points_2d = np.concatenate([points_2d, np.ones((points_2d.shape[0], 1))], axis=1)
    
    A = np.zeros((2 * len(points_2d), 12))
    for i in range(len(points_2d)):
        A[2 * i, :4] = points_3d[i]
        A[2 * i, 8:] = -points_2d[i, 0] * points_3d[i]
        A[2 * i + 1, 4:8] = points_3d[i]
        A[2 * i + 1, 8:] = -points_2d[i, 1] * points_3d[i]
    U, S, V = np.linalg.svd(A)
    P = V[-1].reshape(3, 4)
...
for key, points_2d in zip(["lab_a", "lab_b"], [lab_matches[:, :2], lab_matches[:, 2:]]):
    ...
    print("distance: ", distance, "residual: ", residual)
...
print("residual_lab_a: ", residual_a, "residual_lab_b: ", residual_b)
...
print("residual_library_a: ", residual_a, "residual_library_b: ", residual_b)
...
def fit_fundamental_without_gt(image1, image2):
    ...
    matches = np.concatenate([points1, points2], axis=1)[mask.ravel() == 1]
